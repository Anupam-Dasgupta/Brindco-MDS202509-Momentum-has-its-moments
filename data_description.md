...
Following the final `build_security_identity.py` rerun, treat the actual files and schemas present under `data/processed/security_identity/` as authoritative. This document records the intended/current artifact set but should not override the files on disk.
...
Confirmed script names from the final rebuild workflow include:
